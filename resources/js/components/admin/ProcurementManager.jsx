...
function ProcurementManager() {
    ...
    const fetchRecords = async () => {
        try {
            const response = await fetch(`/api/admin/procurements?search=${encodeURIComponent(search)}&sort_by=date_procured&sort_order=desc`);
    ...
    useEffect(() => { const timer = setTimeout(fetchRecords, 200); return () => clearTimeout(timer); }, [search]);
