...
function ProcurementManager() {
    ...
    const fields = [['date_procured', 'Date Procured', 'date'], ['car_make', 'Car Maker', 'text'], ['car_model', 'Car Model', 'text'], ['car_year', 'Year', 'text'], ['vin', 'VIN', 'text'], ['price_usd', 'Price (USD)', 'number'], ['auction_charge_usd', 'Auction Charge (USD)', 'number'], ['state', 'State', 'text'], ['trucking', 'Trucking', 'number'], ['shipping', 'Shipping', 'text'], ['arrival_date', 'Arrival Date', 'date'], ['profit_ngn', 'Profit (NGN)', 'number'], ['trucking_fee', 'Trucking Fee', 'text']];
    ...
    return <div className="bg-surface-container rounded-xl p-lg border border-white/10">
        ...
        <div className="overflow-x-auto"><table className="min-w-full border-separate border-spacing-0"><thead><tr className="bg-[#0B3D2E] text-left">{['SN', 'DATE PROCURED', 'CAR MAKER', 'CAR MODEL', 'YEAR', 'VIN', 'PRICE (USD)', 'AUCTION CHARGE (USD)', 'AUCTION SITE', 'STATE', 'TRUCKING', 'SHIPPING', 'ARRIVAL DATE', 'PROFIT (NGN)', 'TRUCKING FEE', 'STATUS', 'Actions'].map((heading) => <th key={heading} className="px-md py-sm font-label-md text-white">{heading}</th>)}</tr></thead><tbody className="text-white">{paginatedRecords.map((record, index) => <tr key={record.id} className="border-b border-outline-variant hover:bg-surface-container-highest"><td className="px-md py-sm">{(currentPage - 1) * itemsPerPage + index + 1}</td><td className="px-md py-sm">{record.date_procured || '—'}</td><td className="px-md py-sm">{record.car_make || '—'}</td><td className="px-md py-sm">{record.car_model || '—'}</td><td className="px-md py-sm">{record.car_year || '—'}</td><td className="px-md py-sm">{record.vin || '—'}</td><td className="px-md py-sm">{record.price_usd || '—'}</td><td className="px-md py-sm">{record.auction_charge_usd || '—'}</td><td className="px-md py-sm">{record.auction_site || '—'}</td><td className="px-md py-sm">{record.state || '—'}</td><td className="px-md py-sm">{record.trucking || '—'}</td><td className="px-md py-sm">{record.shipping || '—'}</td><td className="px-md py-sm">{record.arrival_date || '—'}</td><td className="px-md py-sm">{record.profit_ngn || '—'}</td><td className="px-md py-sm uppercase">{record.trucking_fee || '—'}</td><td className="px-md py-sm uppercase">{record.status || '—'}</td><td className="px-md py-sm"><button onClick={() => { setSelectedRecordForInvoice(record); setShowInvoiceModal(true); }} className="text-blue-400 mr-sm">Invoice</button><button onClick={() => edit(record)} className="text-secondary mr-sm">Edit</button><button onClick={() => remove(record.id)} className="text-red-400">Delete</button></td></tr>)}</tbody></table></div>
        ...
        {showInvoiceModal && selectedRecordForInvoice && <InvoiceGenerator service="procurements" record={selectedRecordForInvoice} onClose={() => { setShowInvoiceModal(false); setSelectedRecordForInvoice(null); }} />}
    </div>;
}
