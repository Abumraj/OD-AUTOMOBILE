...
function AdminTopBar({ admin, onLogout, onMenuToggle }) {
    ...
    const performSearch = async () => {
        setIsSearching(true);
        try {
            const response = await fetch(`/api/admin/search-shipments?q=${encodeURIComponent(searchQuery)}`);
                                    ...
                                    <div className="flex items-start justify-between gap-2">
                                        <div className="flex-1">
                                            <div className="flex items-center gap-2 mb-1">
                                                <span className="font-semibold text-on-surface">{result.vehicle_description}</span>
                                                <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                                                    result.status === 'delivered' ? 'bg-green-500/20 text-green-400' :
                                                    result.status === 'in_transit' ? 'bg-blue-500/20 text-blue-400' :
