...
function AutosalesManager() {
    ...
    return (
        <div className="bg-surface-container rounded-xl p-lg border border-white/10">
            ...
            <div className="overflow-x-auto">
                <table className="min-w-full border-separate border-spacing-0">
                    ...
                    <tbody className="text-white">{records.map((record, index) => <tr key={record.id} className="border-b border-outline-variant hover:bg-surface-container-highest">
                        <td className="px-md py-sm">{index + 1}</td><td className="px-md py-sm">{record.sale_date || '—'}</td><td className="px-md py-sm">{record.car_make || '—'}</td><td className="px-md py-sm">{record.car_model || '—'}</td><td className="px-md py-sm">{record.car_year || '—'}</td><td className="px-md py-sm uppercase">{record.sale_type}</td><td className="px-md py-sm">{record.color || '—'}</td><td className="px-md py-sm">{record.vin || '—'}</td><td className="px-md py-sm">{record.amount || '—'}</td><td className="px-md py-sm">{record.profit || '—'}</td><td className="px-md py-sm"><button onClick={() => { setSelectedRecordForInvoice(record); setShowInvoiceModal(true); }} className="text-blue-400 mr-sm">Invoice</button><button onClick={() => edit(record)} className="text-secondary mr-sm">Edit</button><button onClick={() => remove(record.id)} className="text-red-400">Delete</button></td>
                    </tr>)}</tbody>
                </table>
            </div>
            ...
            {showInvoiceModal && selectedRecordForInvoice && <InvoiceGenerator service="autosales" record={selectedRecordForInvoice} onClose={() => { setShowInvoiceModal(false); setSelectedRecordForInvoice(null); }} />}
        </div>
    );
}
