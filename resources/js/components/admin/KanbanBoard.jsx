...
function KanbanBoard() {
  ...
  const columnConfig = [
    ...
    { key: "auction_won", title: "Auction Won", color: "bg-blue-400" },
    { key: "documentation", title: "Documentation", color: "bg-yellow-400" },
    { key: "shipping", title: "Shipping", color: "bg-secondary" },
    { key: "in_transit", title: "In Transit", color: "bg-cyan-400" },
    { key: "customs", title: "Customs", color: "bg-purple-400" },
    { key: "delivered", title: "Delivered", color: "bg-green-400" },
  ];
  ...
  const columns = kanbanData
    ? columnConfig.map((config) => ({
        ...
        cards: (kanbanData[config.key]?.cards || []).map((card) => ({
          ...
          completed: config.key === "delivered",
        })),
      }))
    : [];
      ...
      <div className="flex flex-col h-full min-h-[600px]">
        ...
        <div className="flex space-x-md overflow-x-auto pb-lg">
          {[1, 2, 3, 4, 5].map((i) => (
            <div
              key={i}
              className="flex-shrink-0 w-72 bg-surface-container-low rounded-lg p-sm border border-outline-variant/20 animate-pulse h-96"
          ...
          >
            <div className="flex items-center justify-between mb-md px-1">
              <div className="flex items-center space-x-2">
                ...
                <h3 className="font-title-md text-on-surface text-sm uppercase tracking-wider">
                  {column.title}
                </h3>
              </div>
              ...
              {column.cards.map((card, cardIndex) => (
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    ...
                    {card.delayed && (
                      <span className="bg-error-container text-on-error-container text-[8px] px-1 rounded">
                        DELAY
                      </span>
                    )}
                  </div>
